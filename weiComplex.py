...
def mask_image():
    ...
    if varOS.get() == "Linux":
        ...
        window.update_idletasks()
        window.update()
        os.startfile(entAnacondaDir.get().strip() + "/" + 'pyFAI-drawmask.exe', file)
...
def openPONIfileWindow():
    ...
    def readPoniFile():
        try:
            file = filedialog.askopenfilename(parent=windowPONI, initialdir=os.path(experimentDirectory), filetypes=[("PONI files", ".poni")])
        except:
            file = filedialog.askopenfilename(parent=windowPONI, filetypes=[("PONI files", ".poni")])
        if file is None: 
            return
        ai = pyFAI.load(file)
    ...
    def makePoniFile():
        ...
        for key in paramsFIT2d:
            if len(paramsFIT2d[key]) == 0:
                messagebox.showerror(parent=windowPONI,title="Error", message=key + " not entered")
                return

        wavelength = plankC * speedLightC / 1000 / float(paramsFIT2d["energy"])
        #detector = pyFAI.decorators.Detector(varDetSel.get())
        ai = pyFAI.azimuthalIntegrator.AzimuthalIntegrator(detector=varDetSel.get(), wavelength=wavelength)
        ...
        file = filedialog.asksaveasfile(parent=windowPONI, mode='w', defaultextension=".poni")
    ...
    # labels
    lblHeading = Label(windowPONI, text="Convert FIT2D parameters into PONI format and read .PONI using pyFAI geometry module", font=('TkDefaultFont',15))
    ...
    btnMakePoni = Button(windowPONI,text="Save FIT2d params to PONI file", command=makePoniFile)
    btnReadPoni = Button(windowPONI,text="Read Poni File \n(show FIT2d params)", command=readPoniFile)
...
def makeAIFit2d(**kwargs):
    ...
    ai = azimuthalIntegrator.AzimuthalIntegrator(detector=detector, wavelength=wavelengthcalc)  
    ai.setFit2D(float(directBeam), float(beamX), float(beamY), tilt=float(tilt), tiltPlanRotation=float(tiltPlanRotation))
    
    return ai

def makeFIT2dDic():
    FIT2dParams = {'directBeam': None, 'energy' : None, 'beamX' : None, 'beamY' : None, 'tilt' : None, 'tiltPlanRotation' : None, 'detector' : None }
    return FIT2dParams
...
def readSAXSpar(exp_dir, fname):
    ...
    for count, line in enumerate(lines):
        ...
        if count == 8:
            ...
            FIT2dParams['beamY'] = line.split()[1] # 9m is in correct orientation both on same line
        if count == 9:
...
def readWAXSpar(exp_dir, fname):
    ...
    for count, line in enumerate(lines):
        ...
        if count == 3:
            ...
            FIT2dParams['beamY'] = 1065.0-float(FIT2dParams['beamY']) # correct flip 2d image in theis definition
        if count == 4:
...
def getAI(det_name):
    if det_name == "eiger9m":
        if varPsaxspar.get() == 1:
            ...
            ai = pyFAI.load(experimentDirectory + "/" + entPoni9M.get().strip() )
    if det_name == "eiger1m":    
        if varWaxspar.get() == 1:
            Fit2dParams = readWAXSpar(experimentDirectory, "WAXSpar.txt")
            ai = makeAIFit2d(**Fit2dParams)
        elif varWaxspar.get() == 0:
            ai = pyFAI.load(os.path.join(experimentDirectory, entPoni1M.get().strip() ))
    return ai


def clickGetTransmission():
    getTransmission(entTransAirFrm.get().strip(),entTransSmpFrm.get().strip(),0)
...
def getTransmission(airTMImage, smpTMImage, isBKG):
    ...
    airTransData = fabio.open(os.path.join(experimentDirectory,airTMImage + "_master.h5")).data
    #airTransData = np.multiply(airTransData,mask) #apply mask
    smpTransData = fabio.open(os.path.join(experimentDirectory, smpTMImage + "_master.h5")).data
    ...
    smpBeamCenter = ndimage.measurements.center_of_mass(smpTransData[round(BeamCenY)-pixels:round(BeamCenY)+pixels,round(BeamCenX)-pixels:round(BeamCenX)+pixels])
    smpBeamCenter = [smpBeamCenter[0] + round(BeamCenX)-pixels + 1, smpBeamCenter[1] + round(BeamCenY)-pixels + 1] # need to correct for the size of the centroid image +1 as it starts at 0
    ...
    smpTMNewCen = []
    for num in range(3):
        airInt.append(sum(sum(airTransData[round(BeamCenY)-sumPix[num]:round(BeamCenY)+sumPix[num],round(BeamCenX)-sumPix[num]:round(BeamCenX)+sumPix[num]]))/rigiAir[0])
        smpIntSamCen.append(sum(sum(smpTransData[round(BeamCenY)-sumPix[num]:round(BeamCenY)+sumPix[num],round(BeamCenX)-sumPix[num]:round(BeamCenX)+sumPix[num]]))/rigiSmp[0])
        smpIntNewCen.append(sum(sum(smpTransData[round(smpBeamCenter[1])-sumPix[num]:round(smpBeamCenter[1])+sumPix[num],round(smpBeamCenter[0])-sumPix[num]:round(smpBeamCenter[0])+sumPix[num]]))/rigiSmp[0])
...
def integrateImage(*args):#fileImages, poni, mask,detNum, TM):
    ...
    global scatteringData #data format [[0]name, [1]ID, [2]multi det. flag, [3]sum(I), [4]q, [5]I, [6]err]
    ...
    if detNum == 1 or varDetector.get() == "1M":
        saveDir = uniquify(os.path.join(experimentDirectory,fileNameSmp+"_WC")) # only make for SAXS
        os.mkdir(saveDir)
    else:
        saveDir = lastPath(os.path.join(experimentDirectory,fileNameSmp+"_WC")) # Save WAXS in the same directory    
    
    # read in the image
    if detNum == 1:
        imgSmp = fabio.open(os.path.join(experimentDirectory,fileNameSmp+"_master.h5"))
        if subBkg == 1:
            imgBkg = fabio.open(os.path.join(experimentDirectory,fileNameBkg+"_master.h5"))
    else:
        imgSmp = fabio.open(os.path.join(experimentDirectory,fileNameSmp[1:3]+fileNameSmp[0]+"_master.h5"))
        if subBkg == 1:
            imgBkg = fabio.open(os.path.join(experimentDirectory,fileNameSmp[1:3]+fileNameBkg[0]+"_master.h5"))
    numberFrames = imgSmp.nframes
    print("num frames: " + str(numberFrames))
    maskData = fabio.open(os.path.join(experimentDirectory,mask)).data
    ai = getAI(det_name)
    ...
    for frame in range(numberFrames):
        ...
        #get backgroundframe first if needed (only needed on first loop) and only 1 frame!
        window.update_idletasks()
        if subBkg == 1 and frame == 0:
            if detNum == 1:
                intImBkg = fabio.open(os.path.join(experimentDirectory,fileNameBkg+"_master.h5"))
            else:
                intImBkg = fabio.open(os.path.join(experimentDirectory,fileNameBkg[1:3]+fileNameBkg[0]+"_master.h5"))
        
        if numberFrames == 1:
            if detNum == 1:
                intImSmp = fabio.open(os.path.join(experimentDirectory,fileNameSmp+"_master.h5"))                   
            else:
                intImSmp = fabio.open(os.path.join(experimentDirectory,fileNameSmp[1:3]+fileNameSmp[0]+"_master.h5"))                    
        else:
            intImSmp = imgSmp.getframe(frame)

        # calc normalization value norm value is division
        #civi, thickness, TM, /scaleFactor        
        normValueSmp = civiSmp[frame]*float(entThickness.get().strip())*TMsmp/float(entScaleFactor.get().strip())
        if subBkg == 1:
            normValueBkg = civiBkg[0]*float(entThickness.get().strip())*TMbkg/float(entScaleFactor.get().strip())
            
        if subBkg == 0:
            if detNum == 1:
                saveFileName = os.path.join(saveDir,fileNameSmp+"_"+str(frame)+".dat")
                
            else:
                saveFileName = os.path.join(saveDir,fileNameSmp+"W_"+str(frame)+".dat")
            if varSave.get() == "Save All Data":
                # after checking we can get standard error of mean by using the norm value
                q, I, err = ai.integrate1d(intImSmp.data, num_points, filename=saveFileName, correctSolidAngle=True,variance=None,error_model="poisson",radial_range=None,azimuth_range=None,mask=maskData,dummy=None, delta_dummy=None, polarization_factor=None, dark=None, flat=None, method=intMethod, unit='q_A^-1', safe=False, normalization_factor=normValueSmp, metadata=None)
            else: # don't save
                q, I, err = ai.integrate1d(intImSmp.data, num_points,correctSolidAngle=True,variance=None,error_model="poisson",radial_range=None,azimuth_range=None,mask=maskData,dummy=None, delta_dummy=None, polarization_factor=None, dark=None, flat=None, method=intMethod, unit='q_A^-1', safe=False, normalization_factor=normValueSmp, metadata=None)
                
            if detNum == 1:
                is1M = False
                #scatteringData.append([fileNameSmp+"_"+str(frame), imageCount, multiDet, np.sum(I), q, I, err, TMsmp, is1M,fileNameSmp,frame]) 
                params = {"file name": fileNameSmp+"_"+str(frame), "ID" : imageCount, "multiDet" : multiDet, "sumI" : np.sum(I), "TM" : TMsmp, "is1M" : is1M, "fileNameSmp" : fileNameSmp, "frame" : frame}
                scatteringData.append(expData(q, I, err, params))
            else:
                is1M = True
                params = {"file name": fileNameSmp+"W_"+str(frame), "ID" : imageCount, "multiDet" : multiDet, "sumI" : np.sum(I), "TM" : TMsmp, "is1M" : is1M, "fileNameSmp" : fileNameSmp, "frame" : frame}
                scatteringData.append(expData(q, I, err, params))
            ...
            if detNum == 1:
                is1M = False
                saveFileNameSmp = os.path.join(saveDir,fileNameSmp+"_SUB_"+str(frame)+".dat")
                #saveFileNameBkg = os.path.join(saveDir,"BKG_"+fileNameBkg+"_"+str(frame)+".dat")
                q, ISmp, errSmp = ai.integrate1d(intImSmp.data, num_points, correctSolidAngle=True,variance=None,error_model="poisson",radial_range=None,azimuth_range=None,mask=maskData,dummy=None, delta_dummy=None, polarization_factor=None, dark=None, flat=None, method=intMethod, unit='q_A^-1', safe=False, normalization_factor=normValueSmp, metadata=None)
                if frame == 0: # get the first frame
                    q, IBkg, errBkg = ai.integrate1d(intImBkg.data, num_points, correctSolidAngle=True,variance=None,error_model="poisson",radial_range=None,azimuth_range=None,mask=maskData,dummy=None, delta_dummy=None, polarization_factor=None, dark=None, flat=None, method=intMethod, unit='q_A^-1', safe=False, normalization_factor=normValueBkg, metadata=None)
                ...
                params = {"file name": fileNameSmp+"SUB_"+str(frame), "ID" : imageCount, "multiDet" : multiDet, "sumI" : np.sum(I), "TM" : TMsmp, "is1M" : is1M, "fileNameSmp" : fileNameSmp, "frame" : frame}
                scatteringData.append(expData(q, I, err, params))
                #scatteringData.append([fileNameSmp+"SUB_"+str(frame), imageCount, multiDet, np.sum(I), q, I, err, TMsmp, is1M,fileNameSmp,frame]) 
                # write data to file
                if varSave.get() == "Save All Data":
                    np.savetxt(saveFileNameSmp,np.transpose([q,I,err]), fmt='%1.6e',delimiter='    ' )
 
            else:
                is1M = True
                saveFileNameSmp = os.path.join(saveDir,fileNameSmp+"_SUB_W_"+str(frame)+".dat")
                q, ISmp, errSmp = ai.integrate1d(intImSmp.data, num_points, correctSolidAngle=True,variance=None,error_model="poisson",radial_range=None,azimuth_range=None,mask=maskData,dummy=None, delta_dummy=None, polarization_factor=None, dark=None, flat=None, method=intMethod, unit='q_A^-1', safe=False, normalization_factor=normValueSmp, metadata=None)
                if frame == 0: # get the first frame
                    q, IBkg, errBkg = ai.integrate1d(intImBkg.data, num_points, correctSolidAngle=True,variance=None,error_model="poisson",radial_range=None,azimuth_range=None,mask=maskData,dummy=None, delta_dummy=None, polarization_factor=None, dark=None, flat=None, method=intMethod, unit='q_A^-1', safe=False, normalization_factor=normValueBkg, metadata=None)
                err = np.sqrt(np.add(np.power(errSmp,2),np.power(errBkg,2)))
                ...
                params = {"file name": fileNameSmp+"SUB_W_"+str(frame), "ID" : imageCount, "multiDet" : multiDet, "sumI" : np.sum(I), "TM" : TMsmp, "is1M" : is1M, "fileNameSmp" : fileNameSmp, "frame" : frame}
                scatteringData.append(expData(q, I, err, params))
        ...
        tvData.insert(parent='',index='end',iid=scatteringData[-1].params["ID"], text="", values=(scatteringData[-1].params["ID"],scatteringData[-1].params["file name"],round(scatteringData[-1].params["sumI"],5),round(scatteringData[-1].params["TM"],4)))
        imageCount += 1
        window.update_idletasks()
    ...
    #repopulate()
    try:
        np.array(scatteringData,dtype=object).dump(open(os.path.join(experimentDirectory, entExperimentFile.get().strip()),'wb')) # save the file
    except:
        messagebox.showinfo(title="Error", message="Save data location unknown, \nplease select file > save experiment \n or check advanced tab has the data file with .wc ")
...
def integrateMultiDet():
    ...
    for image in range(len(entSmpSAXSFrm)):
        ...
        if varCalcTMSmp.get() == 1: # if check box is slected calc TM will update GUI once function finishes
            TMsmp = getTransmission(entTransAirFrm[image].get().strip(), entTransSmpFrm[image].get().strip(),0) # calculate the sample transmisison
            #txtsmpTMVal.insert(str(round(TMsmp,5))) # gets put in by other function
            window.update_idletasks()
        else:
            window.update_idletasks()
            TMsmp = float(entsmpTMVal.get().strip()) # otherwise get smp TM from GUI
        
        # do same for background!
        if varCalcTMbkg.get() == 1 and varSubBkg[image].get() == 1: # if check box is slected calc TM will update GUI once function finishes
            TMbkg = getTransmission(entTransAirFrm[image].get().strip(), entTransBkgFrm[image].get().strip(),1) # calculate the background transmisison
            #txtbkgTMVal.insert(str(round(TMbkg,5))) # gets put in by the other function!
            window.update_idletasks()
        elif varCalcTMbkg.get() == 0 and varSubBkg[image].get() == 1: 
            window.update_idletasks()
            TMbkg = float(entbkgTMVal.get().strip()) # otherwise get bkg TM from GUI

        # do the actual integration(s)
        if varSubBkg[image].get() == 0:
            if varDetector.get() == "9M": # no subtract background 9M only
                integrateImage(entSmpSAXSFrm[image].get().strip(), "eiger9m", entMask9M.get().strip(), 1, TMsmp, varSubBkg[image].get())     
            elif varDetector.get() == "1M": # no subtract background 1M only
                integrateImage(entSmpSAXSFrm[image].get().strip(), "eiger1m", entMask1M.get().strip(),2, TMsmp, varSubBkg[image].get())
            elif varDetector.get() == "9M and 1M": # no subtract background 9M and 1M
                integrateImage(entSmpSAXSFrm[image].get().strip(), "eiger9m", entMask9M.get().strip(),1, TMsmp, varSubBkg[image].get())     
                integrateImage(entSmpSAXSFrm[image].get().strip(), "eiger1m", entMask1M.get().strip(),2,TMsmp ,varSubBkg[image].get())
        
        elif varSubBkg[image].get() == 1: # subtract background, FileNameSmp, FileNameBkg, poni, mask, detNum, TM_smp, TM_bkg
            if varDetector.get() == "9M": # sub bkg 9M only
                integrateImage(entSmpSAXSFrm[image].get().strip(), entBkgSAXSFrm[image].get().strip(), "eiger9m",entMask9M.get().strip(), 1, TMsmp, TMbkg, varSubBkg[image].get())     
            elif varDetector.get() == "1M": # sub bkg 1M only
                integrateImage(entSmpSAXSFrm[image].get().strip(), entBkgSAXSFrm[image].get().strip(), "eiger1m",entMask1M.get().strip(),2,TMsmp, TMbkg, varSubBkg[image].get())
...
def loadParams():
    ...
    for line in lines:
        ...
        if  line.split(",")[0] == "experimentFile":
            ...
            try:
                scatteringDataTemp = np.load(open(os.path.join(experimentDirectory,entExperimentFile.get().strip()), 'rb'),allow_pickle=True)
                ...
                messagebox.showinfo(title="Data file read into memory", message=str(len(scatteringData))+ " 1d profiles loaded")
            except:
                messagebox.showinfo(title="Data file not found", message="No file, 0 profiles loaded")    
...
def averageSelected(event=None):
    ...
    # save the file
    try:
        np.array(scatteringData,dtype=object).dump(open(os.path.join(experimentDirectory, entExperimentFile.get()),'wb')) 
...
def subBackground(event=None):
    ...
    I_bkg = np.multiply(scatteringData[int(backGroundID)].I,float(entScaleBkg.get()))
    err_bkg = np.multiply(scatteringData[int(backGroundID)].err,float(entScaleBkg.get()))
    for record in tvData.selection():
        I_smp = np.multiply(scatteringData[int(record)].I,float(entScaleSmp.get()))
        err_smp = np.multiply(scatteringData[int(record)].err,float(entScaleSmp.get()))
        ...
        # save the file
        try:
            np.array(scatteringData,dtype=object).dump(open(os.path.join(experimentDirectory, entExperimentFile.get().strip()),'wb')) 
            tvData.insert(parent='',index='end',iid=scatteringData[-1].params["ID"], text="", values=(scatteringData[-1].params["ID"],
            scatteringData[-1].params["file name"], round(scatteringData[-1].params["sumI"],5),round(scatteringData[-1].params["TM"],4))) # update treeview data
        except:
            messagebox.showinfo(title="Error", message="Save data location unknown, \nplease select file > save experiment \n or check advanced tab has the data file with .wc\nNo data processing performed")
...
def selectBackground(event=None):
    ...
    lblBackground.config(text="Background Frame: " + backGroundName + " ID is " + str(backGroundID))
...
def plotSelectedData(event=None):
    ...
    if len(tvData.selection()) > 20:
        ans = messagebox.askquestion(title="Continue?", message="It is best to plot batches of 10-20, for greater than 20 causes hanging, continue?")
        if ans == 'no':
            return
    ...
    if varAutoScale.get() == 1:
        if len(tvData.selection()) != 2:
            messagebox.showerror('Error', 'Can only auto scale with 1 x 1M and 1x9M')
            return
        flag1M = []
        ID = []
        for record in tvData.selection():
        ...
        if (flag1M[0] == flag1M[1]):
            messagebox.showerror('Error', 'Only 1 1M and 1 9M frame should be selected.')
            return
        # get 9M and 1M ID
        if flag1M[0]:
            ID1M = ID[0]
        ...
        overlap_1M_indices, overlap_9M_indices = find_overlap(scatteringData[ID9M].q[start_index_9M:end_index_9M], scatteringData[ID1M].q[start_index_1M:end_index_1M])
        scale1M = calc_scale_factor(scatteringData[ID9M], scatteringData[ID1M], overlap_1M_indices, overlap_9M_indices)
    ...
    for record in tvData.selection():
        ...
        if varErrorBar.get() == 1:
            if varPlotStyle.get() == "Line":    
                ...
                yerr=np.multiply(scatteringData[int(x)].err[first:last],scale), 
...
def saveSelection(event=None):
    ...
    for record in tvData.selection():
        ...
        if varExportas.get() == 1:
            file = filedialog.asksaveasfile(parent=window, mode='w',initialfile = saveName + ".dat", defaultextension=".dat")
            if file is None: # asksaveasfile return `None` if dialog closed with "cancel".
                return
            np.savetxt(file.name,np.transpose([q,I,err]), fmt='%1.6e',delimiter='    ' )    
        elif varExportas.get() == 0:
            np.savetxt(os.path.join(experimentDirectory,saveName + ".dat"),np.transpose([q,I,err]), fmt='%1.6e',delimiter='    ' )
...
def primusOpen():
    ...
    for record in tvData.selection():
        ...
        err = scatteringData[int(x)].err
        np.savetxt(os.path.join(experimentDirectory,saveName + ".dat"),np.transpose([q,I,err]), fmt='%1.6e',delimiter='    ' )
        files.append(os.path.join(experimentDirectory,saveName + ".dat")) #make text with space between files to be opened by primus 
    hasRun = False
    for item in files:
        if hasRun == False:
            allFiles = item
    ...
    if varOS.get() == "Windows":
        try:
            window.update_idletasks()
            window.update()
            subprocess.popen(os.path.join(entPrimusDir.get().strip(), 'primusqt.exe') + " " + allFiles)
        except:
            messagebox.showerror(title="File not found", message= "Primus file not found")
...
def showPlot2d():
    ...
    if is1M == True:
        ...
        if varMask2d.get() == 1:
            try:
                mask = fabio.open(os.path.join(experimentDirectory,entMask1M.get().strip())).data
            except:
                messagebox.showerror(title="File not found", message= str(os.path.join(experimentDirectory,entMask1M.get().strip()) + " Not found"))
                return
    else:
        ai = getAI("eiger9m")
        if varMask2d.get() == 1:
            try:
                mask = fabio.open(os.path.join(experimentDirectory,entMask9M.get().strip())).data
            except:
                messagebox.showerror(title="File not found", message= str(os.path.join(experimentDirectory,entMask9M.get().strip()) + " Not found"))
                return
        
    if frame == 0:
        img = fabio.open(os.path.join(experimentDirectory,fileName+"_master.h5"))
    else:
        allFrames = fabio.open(os.path.join(experimentDirectory,fileName+"_master.h5"))
    ...
    if varRotate.get() == 1:
        ...
        masked_image = masked_image.rotate(float(entAngle.get().strip()), center=(int(BeamCenX),int(BeamCenY)), resample=IImage.BICUBIC)
    ...
    if varLog2d.get() == 1:
        ...
        plot2d.imshow(np.log10(masked_image), vmin = minColor, vmax = np.log10(maxColor), extent=[qX0,qX1,qZ0,qZ1]) # log 0 problem
    else:
        #plot2d.imshow(np.multiply(img.data,mask))
        plot2d.imshow(np.array(masked_image), vmin = minColor, vmax = maxColor, extent=[qX0,qX1,qZ0,qZ1])
...
def show2dWindow():
    ...
    cbRotate = Checkbutton(window2d,text="Rotate: Ang.",variable=varRotate, onvalue=1, offvalue=0)
    entAngle = Entry(window2d, width=4)
    cbLog2d = Checkbutton(window2d, text="Log intensity", variable=varLog2d, onvalue=1, offvalue=0)
    cbLog2d.select()
    cbApplyMask = Checkbutton(window2d, text="Apply mask", variable=varMask2d, onvalue=1, offvalue=0)
    cbApplyMask.select()
    cbApplyThreshold = Checkbutton(window2d, text="Apply threshold", variable=var2dThreshold, onvalue=1, offvalue=0)
    ...
    def onsize(event):
        ...
        fig2d.set_size_inches((window_width-50)/100, (window_height - 100)/100, forward=True) # 100 dpi
...
def showHelp():
    ...
    Label(helpWindow, font=('TkDefaultFont',15), text= "This is currently not fully tested software, please carefully check any results. \nhttps://sourceforge.net/projects/weicomplex/support for help or to report bugs" ).pack()
...
def clearMemory():
    ...
    try:
        scatteringData = []
        messagebox.showinfo(title="Memory Cleared", message="Scattering data removed from memory.")
    except:
        messagebox.showinfo(title="No data.", message="No scattering data to clear.")
...
def incrementEntryRow():
    ...
    cbSubBkg.append(Checkbutton(frameReduction, text="Sub. Bkg", variable=varSubBkg[-1], onvalue=1, offvalue=0))
    cbSubBkg[-1].select()    
...
def merge(event=None):
    ...
    if varAutoScale.get() == 1:
        ...
        scale = calc_scale_factor(scatteringData[ID9M], scatteringData[ID1M], overlap_1M_indices, overlap_9M_indices)
        entScale1M.delete(0, END)
        entScale1M.insert(END, str(round(scale,8)))
    elif varAutoScale.get() == 0:
        scale = float(entScale1M.get().strip())
    new_data_q = np.append(np.array(scatteringData[ID9M].q[start_index_9M:end_index_9M]), np.array(scatteringData[ID1M].q[start_index_1M:end_index_1M]))
    new_data_I = np.append(np.array(scatteringData[ID9M].I[start_index_9M:end_index_9M]), np.multiply(scatteringData[ID1M].I[start_index_1M:end_index_1M],scale))
    new_data_err = np.append(np.array(scatteringData[ID9M].err[start_index_9M:end_index_9M]), np.multiply(scatteringData[ID1M].err[start_index_1M:end_index_1M],scale))
    ...
    scatteringData.append(expData(new_data_q, new_data_I, new_data_err, params))
...
def onsize(event):
    ...
    if window_width > 1200:
        ...
        fig.set_size_inches((window_width-730)/100, (window_height-110)/100, forward=True) # 100 dpi
    else:
...
calibmenu.add_command(label="PONI <> FIT2D operations", command=openPONIfileWindow)
calibmenu.add_command(label="Calibrate Detector", command=call_pyFAI_calibration)
...
scrollPlottingY = ttk.Scrollbar(framePlotting, orient=VERTICAL, command=canvasPlotting.yview)
scrollPlottingY.pack(side=RIGHT,fill=BOTH)
scrollPlottingX = ttk.Scrollbar(framePlotting, orient=HORIZONTAL, command=canvasPlotting.xview)
...
canvasPlotting.bind('<Configure>',lambda e: canvasPlotting.configure(scrollregion=canvasPlotting.bbox("all")))
...
cbManTMSmp = Checkbutton(frameReduction, text="Calc TM", variable=varCalcTMSmp, onvalue=1, offvalue=0, command=toggle_entry_boxes)
cbManTMSmp.select()
cbManTMBkg = Checkbutton(frameReduction, text="Calc TM", variable=varCalcTMbkg, onvalue=1, offvalue=0, command=toggle_entry_boxes)
cbManTMBkg.select()
cbSubBkg = []
cbSubBkg.append(Checkbutton(frameReduction, text="Sub. Bkg", variable=varSubBkg[-1], onvalue=1, offvalue=0))
cbSubBkg[-1].select()
cbPsaxsPar = Checkbutton(frameReduction, text="pSAXSpar.txt", variable=varPsaxspar, onvalue=1, offvalue=0, command=toggle_entry_boxes)
cbwaxsPar = Checkbutton(frameReduction, text="WAXSpar.txt", variable=varWaxspar, onvalue=1, offvalue=0, command=toggle_entry_boxes)


#frame Plotting
cbLogX = Checkbutton(framePlotting2, text="Log X", variable=varLogX, onvalue=1, offvalue=0)
cbLogX.select()
cbLogY = Checkbutton(framePlotting2, text="Log Y", variable=varLogY, onvalue=1, offvalue=0)
cbLogY.select()
cbErrorBar = Checkbutton(framePlotting2, text="Err. Bars", variable=varErrorBar, onvalue=1, offvalue=0, padx=2)
cbErrorBar.select()
cbGrid = Checkbutton(framePlotting2, text="Grid", variable=varGrid, onvalue=1, offvalue=0)
cbAutoScale = Checkbutton(framePlotting2, text="Auto\nscale", variable=varAutoScale, onvalue=1, offvalue=0)
cbExportas = Checkbutton(framePlotting2, text="Exp. as", variable=varExportas, onvalue=1, offvalue=0)




# Progress bars
progBarSAXS = ttk.Progressbar(frameReduction, orient=HORIZONTAL, length=300, mode='determinate')
progBarWAXS = ttk.Progressbar(frameReduction, orient=HORIZONTAL, length=300, mode='determinate')
...
omBeamPixels = OptionMenu(frameReduction, varBeamPixels, "Beam 3 pixels", "Beam 5 pixels", "Beam 7 pixels")
...
# Main treeview
frame_tvData = LabelFrame(framePlotting2,text="All Data",font=('TkDefaultFont',15))
...
tvData = ttk.Treeview(frame_tvData, yscrollcommand=scroll_tvData.set, selectmode="extended",height=15)
...
lblBackground = Label(framePlotting2, text="Background: Please Select",font=('TkDefaultFont',12))
lblScaleNotice = Label(framePlotting2,text="When Subtracting:", font=('TkDefaultFont',15))
...
lblAnacondaDir = Label(frameAdvanced, text="Anaconda script dir :\n (win OS only)")
...
btn_get_tansmission = Button(frameReduction,text="Get transmission:", command=clickGetTransmission)
btn_integrate_SAXS = Button(frameReduction, text="RUN:",command=integrateMultiDet,font=('TkDefaultFont',20), bg='green', fg ='white')
btn_increase_entry_row = Button(frameReduction, text="Increase row", command=incrementEntryRow)

# plotting
btn_plot_selected = Button(framePlotting2,text="Plot Selected\n(q)",command=plotSelectedData)
btn_av_selection = Button(framePlotting2, text="Average Selection\n(w)", command=averageSelected)
btn_select_bkg = Button(framePlotting2, text="Is Background \n(spacebar)", command=selectBackground)
btn_remove_entry = Button(framePlotting2, text="(R)emove Selection", command=removeEntry)
btn_subtract = Button(framePlotting2, text="(S)ubtract Background \n From Selected Frames", command=subBackground, bg = "cyan")
btn_save_selection = Button(framePlotting2, text="(E)xport Selection", command=saveSelection)
btn_clear_all = Button(framePlotting2, text="Clear All", command=clearAll,font=('TkDefaultFont',12), bg='red', fg ='white')
btn_repopulate = Button(framePlotting2, text="Repopulate", command=repopulate,font=('TkDefaultFont',12), bg='green', fg ='white')
btn_primus = Button(framePlotting2, text="Primus Merge Selected", command=primusOpen)
btn_2d_show_selected = Button(framePlotting2, text="Show in 2d in viewer", command = showPlot2d, bg="orange")
btn_merge = Button(framePlotting2, text="(M)erge", command=merge, font=('TkDefaultFont',12), bg ='snow')
# Frame advanced
btn_clear_memory = Button(frameAdvanced, text="Clear memory", command=clearMemory)
btn_load_experiment = Button(frameAdvanced, text="Load experiment data", command=loadExperiment)
###############
# menu
